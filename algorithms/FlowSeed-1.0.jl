...
function BFS_neighborhood(A::SparseMatrixCSC,R::Array{Int64},setsize::Int64)

    rp = A.rowval
    ci = A.colptr
    n = size(A,1)
    @assert(setsize >= length(R))
    @assert(setsize < size(A,1))
    ...
    while current_size < setsize
        i = pop!(Q)
        neighbs = rp[ci[i]:ci[i+1]-1]
        for j = neighbs
            if eS[j] == 0 && current_size < setsize
                eS[j] = 1
                current_size += 1
                prepend!(Q,j)
            end
...
# This computes the precision, recall, and F1 score for a set Returned
# compared against a Target set
function F2score(Target,Returned)
...
# Get the S-T cut value
function STcutAlpha(A::SparseMatrixCSC{Float64,Int64},S::Vector{Int64},R::Vector{Int64},d::Array{Float64,2},alpha::Float64,epsilon::Float64,volA::Float64,pR::Array{Float64},RinS::Array{Float64})
    ...
    volS = sum(nonzeros(AS));
    edges = (sum(nonzeros(AS[S,:])));
    cutS = volS-edges
...
function FlowSeed(A::SparseMatrixCSC{Float64,Int64},R::Vector{Int64},
    ...
    d::Array{Float64},localFlag::Bool=true)
    ...
    FlowSeed(A,R,Rn,Rc,epsilon,pR,RinS,d,volA,volR,true,localFlag)
