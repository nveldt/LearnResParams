...
# This code uses the parallel projection-based method for solving the LambdaCC LP relaxation.

include("../include/MetricOptimization/ParallelMetricOpt_1.0.jl")
...
function EvaluatePFF(A::SparseMatrixCSC{Float64,Int64},c::Vector{Int64},lam::Float64,dweight::Bool,GapTol::Float64=1e-3,ConTol::Float64=1e-3,gam::Float64=10.0)
    ...
    if dweight
        Output = Parallel_Dykstra_lamCC_dw(A,lam,GapTol,ConTol,gam)
        DualLB = Output[4]
        Denom = DualLB[end]/(1+1/gam)
    else
        Output = Parallel_Dykstra_lamCC_TFA(A,lam,GapTol,ConTol,gam)
        DualLB = Output[4]
        Denom = DualLB[end]/(1+1/gam)
    end
